    ...
    try {
      ...
      data.values.forEach(row => {
        if (row.length >= 3 && row[0] && row[2]) {
          const placa = row[0].toString().trim().toUpperCase();
          const cantidad = parseInt(row[2]) || 4; // Default 4 si no hay dato válido
          
          // Validar que sea 4, 6 o 10
          if ([4, 6, 10].includes(cantidad)) {
            cantidadMap[placa] = cantidad;
          } else {
            cantidadMap[placa] = 4; // Valor por defecto si no es válido
          }
        }
      });

      console.log('Mapa de placas a cantidad de llantas:', cantidadMap);
      return cantidadMap;
